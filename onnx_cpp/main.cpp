...
Ort::Session prepareModel(const std::string modelFilePath)
{   
    string instanceName{"oneclass-onnx"};
    ...
    bool is_get_coco_result = false)
{
    ...
    cv::Mat predictions(INPUT_HEIGHT, INPUT_WIDTH, output_type, output.data());
...
void visualize(const cv::Mat &oriImg, CocoResult result)
{
    ...
    cv::imwrite("../../examples/output_onnx_cpp/mask.jpg", image_bin);
    cv::imwrite("../../examples/output_onnx_cpp/boxes_segment.jpg", showImg);
    cout << "Save mask at: examples/output_onnx_cpp/mask.jpg" << endl;
    cout << "Save segment + boxes at:  examples/output_onnx_cpp/segmented.jpg" << endl;
}
...
int main()
{
    ...
    // configs
    std::string modelFilePath = "../../weights/onnx/model_20221222_9229_664304.onnx";
    std::string img_path = "../../examples/inputs/demo.png";
    const float threshold = 0.1025;
    const float min_area_ratio = 1.22e-05;
    ...
    session.Run(
        ...
        inputNames.data(),
        inputTensors.data(), 1 /*Num of inputs*/,
        outputNames.data(),
        outputTensors.data(), 1 /*Num of outputs*/
    );
    // post processing
    ...
    auto timeInfer = (timer::now() - lastTime);
    cout << "Time Infer: " << std::chrono::duration_cast<std::chrono::milliseconds>(timeInfer).count() << "ms" << endl;
